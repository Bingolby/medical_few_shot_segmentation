...
class MedicalEpisodicData(Dataset):
    def __init__(self,
                 transform: transform.Compose,
                 class_list: List[int],
                 data_list_path: str,
        ...
        self.data_root = args.data_root
        self.class_list = class_list
        data_list_path_q = data_list_path
        data_list_path_s = data_list_path.split('/')[:-1]
        data_list_path_s.append('val_s.txt')
        data_list_path_s = '/'.join(data_list_path_s)
        # print(args.data_root, data_list_path, self.class_list)#data/cervix2digestpath/ lists/cervix2digestpath/val_q.txt [1]
        self.data_list_q, self.sub_class_file_list_q = make_dataset(args.data_root, data_list_path_q, self.class_list)
        self.data_list_s, self.sub_class_file_list_s = make_dataset(args.data_root, data_list_path_s, self.class_list)
        self.transform = transform

    def __len__(self):
        return len(self.data_list_q)

    def __getitem__(self, index):

        # ========= Read query image + Chose class =========================
        label_class = []
        image_path, label_path = self.data_list_q[index]
        image = cv2.imread(image_path, cv2.IMREAD_COLOR)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        image = np.float32(image)
        label = cv2.imread(label_path, cv2.IMREAD_GRAYSCALE)
        ...
        file_class_chosen = self.sub_class_file_list_s[class_chosen]
        num_file = len(file_class_chosen)
